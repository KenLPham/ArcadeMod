...
public class BlockPlushie extends Block implements IBlockVariant {
	...
	private static final AxisAlignedBB[][] boundingBox = { // [Mob][Rotation]
			{ // Creeper TODO: Fix Bounding Box (Rotation Update)
					new AxisAlignedBB((5.0D / 16.0D), 0.0D, (5.0D / 16.0D), (11.0D / 16.0D), (13.0D / 16.0D), (11.0D / 16.0D)) }, { // Pig TODO: Correct Bounding Box
			new AxisAlignedBB((5.0D / 16.0D), 0.0D, (5.0D / 16.0D), (11.0D / 16.0D), (10.0D / 16.0D), (11.0D / 16.0D)) } };
	
	private static final SoundEvent[] mobSounds = { SoundEvents.ENTITY_CREEPER_HURT, SoundEvents.ENTITY_PIG_AMBIENT };
	...
	public boolean onBlockActivated (World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (tile instanceof TileEntityPlushie) {
			TileEntityPlushie plushie = (TileEntityPlushie)tile;
			if (world.isRemote)
				world.playSound(player, pos, mobSounds[plushie.getMobID()], SoundCategory.BLOCKS, 1.0F, 1.0F); // TODO: Volume based on distance
		}
		return true;
	}
