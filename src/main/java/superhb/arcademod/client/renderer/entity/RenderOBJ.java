...
public abstract class RenderOBJ<T extends Entity> extends Render<T> {
	private IBakedModel[] bakedModels;
	
	protected RenderOBJ (RenderManager manager) {
		super(manager);
	}
	...
	protected abstract boolean preRender (T entity, int model, BufferBuilder buffer, double x, double y, double z, float yaw, float partialTicks);
	...
	public void doRender (T entity, double x, double y, double z, float yaw, float partialTicks) {
		if (this.bakedModels == null) {
			...
			for (int i = 0; i < this.getEntityModels().length; ++i) {
				...
				if (bakedModel instanceof OBJModel.OBJBakedModel && model instanceof OBJModel) {}
					//this.bakedModels[i] = new BakedMWItem((OBJModel)model, ((OBJModel.OBJBakedModel)bakedModel).getState(), DefaultVertexFormats.ITEM, ClientProxy.getTextures((OBJModel)model));
			}
		}
		...
		for (int i = 0; i < this.bakedModels.length; ++i) {
			...
			if (this.preRender(entity, i, buffer, x, y, z, yaw, partialTicks)) {
				int color = this.getColor(i, entity);
				GlStateManager.rotate(-(entity.prevRotationPitch + (entity.rotationPitch - entity.prevRotationPitch) * partialTicks), 1.0F, 0.0F, 0.0F);
				
				BlockPos pos = new BlockPos(entity.posX, entity.posY, entity.posZ);
				EnumFacing facing = null; //entity instanceof EntityWidowmakerMine ? ((EntityWidowmakerMine)entity).facing : entity instanceof EntityJunkratMine ? ((EntityJunkratMine)entity).facing : null;
				if (facing != null) {
				...
				} //else if (entity instanceof EntityReinhardtStrike) pos = pos.add(0, 255, 0);
