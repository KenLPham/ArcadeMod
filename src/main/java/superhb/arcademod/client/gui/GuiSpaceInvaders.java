...
public class GuiSpaceInvaders extends GuiArcade {
	...
	public void updateScreen () {
		...
		if (inMenu) {
			...
			if (playing) {
				if (theme == null) {
					theme = new LoopingSound(this.getTileEntity(), ArcadeSoundRegistry.SPACEINVADERS, SoundCategory.RECORDS, volume - 0.5f);
				}
				if (!mc.getSoundHandler().isSoundPlaying(theme)) {
	...
	public void drawScreen (int mouseX, int mouseY, float partialTick) {
		...
		if (inMenu) {
			switch (menu) {
				case 0: // Main Menu
					int titleWidth = this.fontRendererObj.getStringWidth(I18n.format("game.arcademod:spaceinvaders.name"));
					int startWidth = this.fontRendererObj.getStringWidth(I18n.format("option.arcademod:start.locale"));
					
					this.fontRendererObj.drawString(I18n.format("game.arcademod:spaceinvaders.name"), boardX + (GUI_X / 2) - (titleWidth / 2), boardY + 2, Color.WHITE.getRGB());
				...
				case 1: // Controls
					...
					// Controls
					this.fontRendererObj.drawString("[" + KeyHandler.up.getDisplayName() + "] " + I18n.format("control.arcademod:up.spaceinvaders.name"), boardX + (GUI_X / 2) - 30, boardY + (GUI_Y / 2) - 20, Color.WHITE.getRGB());
			...
			// Text
			if (!playing) {
				this.fontRendererObj.drawString(I18n.format("text.arcademod:ready.spaceinvaders.locale"), boardX + (MAZE_X / 2) - (this.fontRendererObj.getStringWidth(I18n.format("text.arcademod:ready.spaceinvaders.locale")) / 2), boardY + (MAZE_Y / 2) + 13, Color.yellow.getRGB());
			}
			
			this.fontRendererObj.drawString(String.format(I18n.format("text.arcademod:score.locale") + " %d", score), boardX + 10, boardY + 6, Color.white.getRGB());
			...
			this.fontRendererObj.drawString(String.format(I18n.format("text.arcademod:level.locale") + " %d", level), boardX + 170, boardY + 6, Color.white.getRGB());
		}
	}
			...
			for (int j=0; j < 8; j++) {
				...
				if (bullet != null) {
					if (bullet.extendedX >= (aliens[id].extendedX - 7) &&
						...
						bullet = null;
						score += 10;
						getWorld().playSound(getPlayer(), getPos(), ArcadeSoundRegistry.SPACEINVADERS_DESTROYED, SoundCategory.BLOCKS, volume - 0.2f, 1.0f);
					}
				}
			}
		}
		...
		if (inMenu) {
			...
			if (keyCode == KeyHandler.left.getKeyCode()) {
				...
				if (bullet == null) {
					...
					bullet.canMove = true;
					getWorld().playSound(getPlayer(), getPos(), ArcadeSoundRegistry.SPACEINVADERS_SHOOT, SoundCategory.BLOCKS, volume, 1.0f);
				}
			}
	...
	private class Player extends Mover {
		...
		public void kill () {
			...
			deathTick = tickCounter;
			getWorld().playSound(getPlayer(), getPos(), ArcadeSoundRegistry.SPACEINVADERS_EXPLODE, SoundCategory.BLOCKS, volume, 1.0f);
		}
